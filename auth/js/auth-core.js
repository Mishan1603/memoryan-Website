...
class MemoryanAuth {
  ...
  displayBlockingOverlay(message, redirectUrl, delayMs = 5000) {
    ...
    /**
     * Open/close language dropdown in a bulletproof way for Safari:
     * when open, add a full-screen backdrop above the page and the dropdown above the backdrop,
     * both as direct children of body with maximum z-index so nothing can cover the menu.
    ...
    positionLanguageDropdown(open) {
        ...
        const BACKDROP_ID = 'auth-lang-dropdown-backdrop';
        ...
        if (open) {
            ...
            if (!backdrop) {
                ...
                backdrop.addEventListener('click', () => this.positionLanguageDropdown(false));
            }
            document.body.appendChild(backdrop);
            document.body.appendChild(dropdown);
            dropdown.style.cssText = 'position:fixed;top:' + (rect.bottom + 10) + 'px;right:' + (window.innerWidth - rect.right) + 'px;left:auto;z-index:' + Z_TOP + ';';
            ...
            if (dropdown.parentNode !== selector) selector.appendChild(dropdown);
